...
% uncomment one the following two lines, first one showing pictures
\documentclass[conference]{acmsiggraph}
%\documentclass[annual,draft]{acmsiggraph}

\usepackage{subcaption}
\usepackage{color}

% comment out this line to get figures; uncomment for fast latex
\usepackage{graphicx}
...
\author{Mark D. Benjamin \\ Princeton University \and 
Stephen DiVerdi \\ Google \and 
Adam Finkelstein \\ Princeton University}

\pdfauthor{Mark D. Benjamin, Stephen DiVerdi, Adam Finkelstein}
...
\teaser{
  ...
  \caption{\emph{Left:} A vector painting made in our system exhibits both smooth gradients painted by soft brushes as well as infinitely sharp boundaries painted by hard brushes. \emph{Middle:} The painting is represented by a planar triangle mesh. \emph{Right:} Closeup showing the bottom of the letter B reveals both the sharp boundaries on the white region and a smooth gradient from blue to green in the background.}
  \label{fig:teaser}
}
...
our system uses a combination of rasterization and marching squares to
...
Work has also been done in multi-resolution painting using raster graphics. \modified{Early work with Pad++ by Bederson and Hollan~\shortcite{Bederson:1994:PZG:192426.192435} 
explored the possibility of having an infinite resolution canvas. However, rather than a painting program, 
this work focused on creating an interface builder for developers.}
The method of Berman~\etal~\shortcite{Berman:1994:MPC:192161.192181}
...
same limitations. \modified{One way to reduce blurriness in raster images of sharp features is proposed by Ramanarayanan~\etal~\shortcite{ramanarayanan2004feature}. 
By seprately storing where important boundaries are, their system can avoid excessive interpolation. However, such a method requires
creating such a data structure with those stored boundaries.} Another multi-resolution approach by Perlin and Velho~\shortcite{Perlin:1995:LPP:218380.218437} uses
...
\begin{figure}[t]
    \centering
        \fbox{\includegraphics[width=0.45\textwidth]{images/gradientarcs}}
...
rendered to yield a rasterized version of the stroke. \modified{This rasterization happens at the resolution of the current window, 1280x960. Therefore
whatever the artist sees on the screen is the basis for the vectorization.}
...
greedily eliding vertices whenever they lie within a small threshold distance of $P$. \modified{We use
a threshold of 0.5 pixels at the current zoom level, which works well empirically.}
...
\begin{figure}
    \centering
        \fbox{\includegraphics[width=.22\textwidth]{images/emptycanvas}}
        \fbox{\includegraphics[width=.22\textwidth]{images/canvaswithonestroke}}
...
First we consider the case of a hard brush, which contains a single contour (\fig{fig:rastertotriangles}-left).
...
derived from the contours are triangulated as in \sect{sec:contours}.
The resulting mesh represents the soft
...
all triangles connecting the inner and outer strokes a linear gradient as seen in Figure~\ref{fig:softarcs}.
Figure~\ref{fig:rastertotriangles}-right shows the process for triangulating soft strokes.
...
to the list of points and boundaries already stored on the canvas. 
%
These points and boundaries are all triangulated using constrained Delaunay triangulation
as in \sect{sec:contours}, 
to produce a new mesh (Figure~\ref{fig:firststrokes}).
%
The new mesh contains all of the points of the old and new strokes. 
Since boundary edges form constraints in the triangulation, 
stroke edges are preserved. This means that
no triangle will cross the boundary of a stroke. 
Without this constraint, it would be impossible to correctly color the
triangles since they would straddle color arcs. 

\subsection{Determining Colors}

Once a new mesh is formed the color arcs of the 
...
arc describes the inside of the stroke, which takes the color of the stroke
composited over the color of the canvas at that point. The other arc describes the outside
of the stroke, which is just the color of the canvas at that point. The
...
This arrangement produces a linear gradient across the triangles separating the inner
and outer contours.

The color arcs for old points remain unchanged, unless part of the new stroke covers it.
...
When the constrained Delaunay triangulation is computed
for a set of points with intersecting boundary edges, it must
insert a new point at the intersection of the edges
(otherwise at least one triangle would have to
cross a boundary). These points are introduced with no color information, 
whereas the final colors for 
...
other points on the boundaries. For both boundaries, 
we search both directions to the first point with color data. Note this may not
be the first point on each side, since this intersection point may be adjacent to 
\emph{other} intersection
...
% XXX
For an example of how color arcs are determined for both intersection and regular points see Figure~\ref{fig:colorarcdeterminer}.

\subsection{Finding Modified Points}

As described so far, our retriangulation procedure has two substantial drawbacks. 
%
First, it processes many points that do not need consideration which is inefficient. 
Second, since it acts on the whole
canvas, local modifications may have global effects. 
%
Instead our approach is to determine
which triangles can remain unchanged
...
is generated and all of the cells that contain a triangle from the new stroke are marked. 
...
and their union with the new triangle mesh represents the new canvas.
...
%
The images shown in \fig{fig:boxes}
...
\modified{Nevertheless, this range far exceeds limits
imposed by existing vector graphics software such as Adobe Illustrator, which has a maximum zoom ratio
of 64:1.  Furthermore, the ability to make paint strokes of smooth gradients in a flat vector representation (\eg, Figure~\ref{fig:teaser}) differentiates our approach from previous methods.}
...
results in more geometry and therefore takes longer to \modified{process this new stroke}. The zooming case explores different
...
Figure~\ref{fig:timing} gives a similar result for the cumulative time strokes take to process \modified{after mouse up}. Both
the zooming and non-zooming cases grow non-linearly over time. However, the non-zooming case takes
longer for the reasons described. \modified{For example, the first ten strokes in both cases take about 2 seconds each
to process, while the last ten strokes take about 20 seconds each in the non-zooming case and 7 seconds each in the zooming case.}
...
Performance is measured on a 1.8 GHz Core i5 MacBook Air with 8GB of RAM and an Intel
HD 4000 graphics card.  The current implementation uses Python and PyOpenGL. Python's performance is slower than
...
that geometry can take several seconds to process. Some of this could be alleviated by a carefully optimized implementation. However, there are many calculations
...
% This brings us to the final point, simplification.
...
%
Finally we note that Liao~\etal~\shortcite{10.1109/TVCG.2012.76} demonstrated that this kind of simplification can be effective at reducing mesh complexity, at least for natural imagery.
%
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.5\columnwidth}
    ...
    \label{fig:timing}
  \end{subfigure}%
  \caption{For paintings of 50 random strokes, the left graph shows the number of triangles in the canvas as of stroke $x$, and the right graph shows the cumulative process time of the painting. The blue points are the non-zooming case, while the red points are the zooming case.}
\end{figure}
...
\begin{figure*}
    \centering
        \fbox{\includegraphics[width=0.49\textwidth]{images/facezoom}}
        \fbox{\includegraphics[width=0.49\textwidth]{images/facezoomtriangles}}
    \caption{This drawing uses about 70 strokes, resulting in 9030 triangles.}
    \label{fig:face}
\end{figure*}
...
\begin{figure*}
    \centering
        \fbox{\includegraphics[height=0.333\textwidth]{images/apple}}
		\hspace*{0.5in}
        \fbox{\includegraphics[height=0.333\textwidth]{images/eye}}
...
\begin{figure*}
    \centering
        \fbox{\includegraphics[width=0.325\textwidth]{images/stop12}}
        \fbox{\includegraphics[width=0.325\textwidth]{images/stop22}}
        \fbox{\includegraphics[width=0.325\textwidth]{images/stop32}}
...
Aside from the {\bf simplification} challenge described in \sect{sec:simplify}, there are a number of other interesting areas for future work, including:
