...
a threshold relative to zoom of 0.5 which was determined to work well empirically.}
...
this could be ameliorated by switching to double-precision. \modified{Traditional vector
graphics software such as Adobe Illustrator caps zooming at a ratio of 64:1. Although this
is not an inherent limitation, there is currently not a way to use such vector graphics programs
to draw using a traditional painting metaphor with gradients while so zoomed in.}
...
\modified{Note that all these numbers come from running the program on a 1.8 GHz Core i5 MacBook Air with 8GB of RAM and an Intel HD 4000 graphics card.}
...
utilizing the GPU. For example, Nehab and Hoppe~\shortcite{Nehab:2008:RRG:1409060.1409088} demonstrate an
approach for using GPUs to speed up rending of more traditional vector graphics.}
